<?php
		...
		echo "<div class='side_block'>";
			...
			foreach ( $GLOBALS['global_region_array'] as $region => $region_details ) {
				...
				foreach ( $order_server_array as $server => $server_average ) {
					...
					$country 			= get_image_flag($country);
    ...
    function generate_server_fields($server_details, $text_limit) {
        ...
        $server_details['name']         = "$country <span>".generate_hyperlink_server($server_details['name'], "world", 0, $server_details['faction'], "")."</span>";
	...
	function get_server_ranking_details($server_details, $guild_array, $region_total) {
		...
		foreach( $guild_array as $guild_id => $guild_details ) {
			if ( $guild_details['server'] == $server_details['name'] ) {
				if ( $guild_details['faction'] == "Dominion" ) { $num_of_dom++; }
				if ( $guild_details['faction'] == "Exiles" ) { $num_of_exile++; }
				if ( $guild_details['rank'] <= 25 ) { $num_of_top_25++; }
				if ( $guild_details['rank'] <= 50 ) { $num_of_top_50++; }
		...
		if ( isset($region_total[$region]['num_of_dom']) ) { $region_total[$region]['num_of_dom'] += $num_of_dom; } else { $region_total[$region]['num_of_dom'] = $num_of_dom; }
		if ( isset($region_total[$region]['num_of_exile']) ) { $region_total[$region]['num_of_exile'] += $num_of_exile; } else { $region_total[$region]['num_of_exile'] = $num_of_exile; }
		if ( isset($region_total[$region]['top_25']) ) { $region_total[$region]['top_25'] += $num_of_top_25; } else { $region_total[$region]['top_25'] = $num_of_top_25; }
		if ( isset($region_total[$region]['top_50']) ) { $region_total[$region]['top_50'] += $num_of_top_50; } else { $region_total[$region]['top_50'] = $num_of_top_50; } 

		if ( $region_total[$region]['num_of_guilds'] > 0 ) {
			$region_total[$region]['num_of_dom_perc'] 	= number_format(($region_total[$region]['num_of_dom']/$region_total[$region]['num_of_guilds'])*100, 0, ".", ",")."%";
			$region_total[$region]['num_of_exile_perc'] = number_format(($region_total[$region]['num_of_exile']/$region_total[$region]['num_of_guilds'])*100, 0, ".", ",")."%";
		} else {
			$region_total[$region]['num_of_dom_perc'] 	= "0%";
			$region_total[$region]['num_of_exile_perc'] 	= "0%";			
		}

		// Server Details
		if ( $num_of_dom > $num_of_exile ) { $server_details['faction'] = "Dominion"; }
		if ( $num_of_exile > $num_of_dom ) { $server_details['faction'] = "Exiles"; }
		if ( $num_of_guilds == 0 || $num_of_exile == $num_of_dom ) { $server_details['faction'] = "Neutral"; } 

		if ( $num_of_guilds > 0 ) {
			$num_of_dom 	= number_format(($num_of_dom/$num_of_guilds)*100, 0, ".", ",")."%";
			$num_of_exile 	= number_format(($num_of_exile/$num_of_guilds)*100, 0, ".", ",")."%";
		} else {
			$num_of_dom 	= "0%";
			$num_of_exile 	= "0%";			
		}

		$server_details['guilds'] 			= "{$num_of_guilds} (<a class='dominion'>{$num_of_dom}</a>/<a class='exiles'>{$num_of_exile}</a>)";
		...
		$server_details['top_25'] 			= $num_of_top_25;
		$server_details['top_50'] 			= $num_of_top_50;

		$region_total[$region]['guilds'] 	= "{$region_total[$region]['num_of_guilds']} (<a class='dominion'>{$region_total[$region]['num_of_dom_perc']}</a>/<a class='exiles'>{$region_total[$region]['num_of_exile_perc']}</a>)";
